function qnew = DiffusionSolveMain2(q,Qbc)
    ...
    rate = 2*ones(1,11);
    i = 2;
    while error(i-1)>1e-7 && max(rate(:,i-1))>1.1 && i < 11

        B1 = zeros(64,32);
        B1(49:64,1) = (q2(1:16,32) + q2(1:16,33))*dt/Re/dy/dy/2;
        B1(64,1:32) = B1(64,1:32) + (q2(16,33:64) + q2(17,33:64))*dt/Re/dx/dx/2;
        q1 = DiffusionSolveModified(q(1:64,33:64),Qbc(1:64,33:64)+B1);
        
        B2 = zeros(32,64);
        B2(1,33:64) = (q1(48,1:32) + q1(49,1:32))*dt/Re/dx/dx/2;
...
%     plot(max(rate(:,3:i)));
    ...
    fprintf(1,'Diffusion: iter: %i error: %g rate: %g \n',i,error(i-1),rate(i-1));
    qnew = zeros(80,64);
    qnew(1:64,33:64) = q1;
    qnew(49:80,1:64) = q2;
    qnew(1:48,1:32) = NaN;
